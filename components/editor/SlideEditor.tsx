...
import { SlideData, SlideLayout, SlideItem, SlideNumbering } from '../../types/index';
import { 
  Type, Image, Table, GitCommit, AlignLeft, 
  AlignCenter, AlignRight, Quote, Grid, Hash, Sun, Moon,
  Plus, Trash2, Upload, PlayCircle, Bookmark, Heading, ImageIcon,
  Columns, Baseline, GalleryHorizontal, Briefcase
} from 'lucide-react';
...
const SlideEditor: React.FC<SlideEditorProps> = ({ slide, onUpdate, onGroupUpdate }) => {
  ...
  const handleApplyFontToAll = () => {
      if (onGroupUpdate && slide.fontFamily) onGroupUpdate({ fontFamily: slide.fontFamily });
  };

  const handleApplyFontSizeToAll = () => {
      if (onGroupUpdate && slide.fontSize) onGroupUpdate({ fontSize: slide.fontSize });
  };

  const handleApplyTitleFontSizeToAll = () => {
      if (onGroupUpdate && slide.titleFontSize) onGroupUpdate({ titleFontSize: slide.titleFontSize });
  };

  const handleApplyNumberingToAll = () => {
      if (onGroupUpdate && slide.slideNumbering) onGroupUpdate({ slideNumbering: slide.slideNumbering });
  };

  const handleApplyThemeToAll = (theme: 'light' | 'bri') => {
      if (onGroupUpdate) onGroupUpdate({ themeMode: theme });
  };
    ...
    <div className="bg-white h-full overflow-y-auto border-l border-zinc-200">
      ...
      <div className="sticky top-0 bg-white/95 backdrop-blur z-10 px-6 py-4 border-b border-zinc-100 flex justify-between items-center">
        ...
        <div className="flex gap-2">
             <button 
                onClick={() => handleApplyThemeToAll('light')} 
                className={`flex items-center gap-1.5 px-2 py-1 rounded-sm border text-[10px] font-bold uppercase tracking-wider transition-all ${slide.themeMode !== 'bri' ? 'bg-zinc-100 text-black border-zinc-200' : 'text-zinc-400 border-transparent hover:bg-zinc-50'}`}
                title="Default Architecture Theme"
            >
                <Sun size={10} /> Default
            </button>
            <button 
                onClick={() => handleApplyThemeToAll('bri')} 
                className={`flex items-center gap-1.5 px-2 py-1 rounded-sm border text-[10px] font-bold uppercase tracking-wider transition-all ${slide.themeMode === 'bri' ? 'bg-[#0857C3] text-white border-[#0857C3]' : 'text-zinc-400 border-transparent hover:bg-zinc-50'}`}
                title="BRI Corporate Theme"
            >
                <Briefcase size={10} /> BRI
            </button>
        </div>
      </div>
      ...
      <div className="p-6 space-y-8">
        ...
        <div className="space-y-5">
            <div className="space-y-3">
                <div className="flex justify-between items-center">
                    ...
                    <button onClick={handleApplyFontToAll} className="text-[9px] text-zinc-400 hover:text-black hover:underline disabled:opacity-30" disabled={!slide.fontFamily} title="Apply Font Family to All">Apply Family</button>
                </div>
                <select value={slide.fontFamily || ''} onChange={(e) => handleChange('fontFamily', e.target.value || undefined)} className="w-full px-2 py-1.5 bg-zinc-50 border border-zinc-200 rounded-sm text-xs focus:outline-none focus:border-zinc-400">
                ...
                <div className="flex gap-2">
                    <div className="flex-1">
                        ...
                        <select value={slide.titleFontSize || ''} onChange={(e) => handleChange('titleFontSize', e.target.value || undefined)} className="w-full px-2 py-1.5 bg-zinc-50 border border-zinc-200 rounded-sm text-xs focus:outline-none focus:border-zinc-400">
                    ...
                    <div className="flex-1">
                        ...
                        <select value={slide.fontSize || 'md'} onChange={(e) => handleChange('fontSize', e.target.value)} className="w-full px-2 py-1.5 bg-zinc-50 border border-zinc-200 rounded-sm text-xs focus:outline-none focus:border-zinc-400">
             ...
             <div className="space-y-2">
                 <div className="flex justify-between items-center">
                    ...
                    <button onClick={handleApplyNumberingToAll} className="text-[9px] text-zinc-400 hover:text-black hover:underline disabled:opacity-30" disabled={!slide.slideNumbering?.enabled}>Apply to All</button>
                </div>
                <div className="flex items-center gap-2 mb-2">
                    <label className="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" className="sr-only peer" checked={slide.slideNumbering?.enabled || false} onChange={(e) => handleNumberingChange('enabled', e.target.checked)}/>
                ...
                {slide.slideNumbering?.enabled && (
                    <div className="grid grid-cols-2 gap-2">
                         <select value={slide.slideNumbering.position} onChange={(e) => handleNumberingChange('position', e.target.value)} className="w-full px-2 py-1 bg-zinc-50 border border-zinc-200 rounded-sm text-[10px] focus:outline-none">
                            <option value="bottom-right">Bottom Right</option>
                            <option value="bottom-left">Bottom Left</option>
                            <option value="top-right">Top Right</option>
                            <option value="top-left">Top Left</option>
                        </select>
                        <select value={slide.slideNumbering.format} onChange={(e) => handleNumberingChange('format', e.target.value)} className="w-full px-2 py-1 bg-zinc-50 border border-zinc-200 rounded-sm text-[10px] focus:outline-none">
        ...
        <div className="space-y-5">
            <div className="space-y-2">
                ...
                <FormattingToolbar onInsertTag={insertTag} focusedField={focusedInput?.field} />
